...
class InfluxDBDatabaseSizeRunner:
    ...
    def __call__(self, es, params):
        ...
        * ``index``: the name of the database to get size of.
        ...
        self._influxdb_client.switch_database("_internal")

        query = 'q=SELECT "diskBytes", "writePointsOk" FROM "shard" WHERE "database"="{}" GROUP BY "database" ORDER BY time DESC LIMIT 1'.format(database)

        response = self._influxdb_client.request("query", data=query)
